    ...
    <div class="container">
        ...
        <div class="category-grid<?php echo esc_attr($column_class); ?>">
            <?php foreach ($categories as $category) : 
                $category_icon_type = get_theme_mod('category_icon_type_' . $category->term_id, 'image');
                $category_icon = get_theme_mod('category_icon_' . $category->term_id, '');
                $category_fontawesome = get_theme_mod('category_fontawesome_' . $category->term_id, '');
                $category_link = get_category_link($category->term_id);
                $post_count = $category->count;
            ?>
                <div class="category-item">
                    <a href="<?php echo esc_url($category_link); ?>" class="category-link">
                        <?php if ($category_icon_type === 'fontawesome' && !empty($category_fontawesome)) : ?>
                            <div class="category-icon category-icon-fa">
                                <i class="<?php echo esc_attr($category_fontawesome); ?>"></i>
                            </div>
                        <?php elseif ($category_icon_type === 'image' && !empty($category_icon)) : ?>
